...
Vue.use(Shortkey, {
  prevent: [".disableshortkey", ".disableshortkey textarea"]
});
const VM = template =>
  new Vue({
    template,
...
describe("Index.js", () => {
  describe("fn.decodeKey", () => {
    it("Return single key", () => {
      ...
      expect(Shortkey.decodeKey({ key: "ContextMenu" })).to.equal(
        "contextmenu"
      );
      expect(Shortkey.decodeKey({ key: "ScrollLock" })).to.equal("scrolllock");
      expect(Shortkey.decodeKey({ key: "BrowserHome" })).to.equal(
        "browserhome"
      );
      expect(Shortkey.decodeKey({ key: "MediaSelect" })).to.equal(
        "mediaselect"
      );
    });
    ...
    it("Return a combined key", () => {
      ...
      expect(
        Shortkey.decodeKey({ altKey: true, ctrlKey: true, key: "a" })
      ).to.equal("ctrlalta");
    });
  });
});
...
describe("functionnal tests", () => {
  ...
  describe("Dispatch triggered event", () => {
    ...
    it("listen for keydown and dispatch event with object key", done => {
      const vm = new VM(
        "<div @shortkey=\"foo\" v-shortkey=\"{option1: ['q'], option2: ['a']}\"></div>"
      );
  ...
  describe("Don`t dispatch triggered event", () => {
    it("dont trigger listen for keydown and dispatch event", () => {
      const vm = new VM(
        '<div @shortkey="foo" v-shortkey="[\'b\']"><textarea v-shortkey.avoid></textarea></div>'
      );
      vm.$mount(createDiv());
    ...
    it("listen for keydown and dispatch event with object key", () => {
      const vm = new VM(
        "<div @shortkey=\"foo\" v-shortkey=\"{option1: ['q'], option2: ['a']}\"><textarea v-shortkey.avoid></textarea></div>"
      );
      vm.$mount(createDiv());
  ...
  it("Setting focus with .focus modifier", () => {
    const vm = new VM(
      `<div><input type="text" /> <button type="button" v-shortkey.focus="['f']">BUTTON</button></div>`
    );
    vm.$mount(createDiv());
  ...
  it("Bring push button with .push modifier", () => {
    const vm = new VM(
      `<div><button type="button" v-shortkey.push="['p']" @shortkey="foo()">BUTTON</button></div>`
    );
    vm.$mount(createDiv());
  ...
  it("Testing ? key", () => {
    const vm = new VM(
      `<button @shortkey="foo" v-shortkey="['shift', '?']"></button>`
    );
    vm.$mount(createDiv());
  ...
  it("Prevent bubble event", () => {
    const vm = new VM(
      '<div @shortkey="bar" v-shortkey="[\'a\']"><button type="button" @shortkey="foo" v-shortkey="[\'b\']">TEST</button></div>'
    );
    vm.$mount(createDiv());
  ...
  describe("with prevent list as function", () => {
    before(() => {
      Shortkey.updateOptions({
        prevent: decodedKey => {
          if (decodedKey == "c") {
            return [];
          }
          return [".disableshortkey", ".disableshortkey textarea"];
        }
      });
    });
    ...
    it("does not trigger events when its class is in the prevent list", () => {
      const vm = new VM(
        `<div @shortkey="foo" v-shortkey="{b: ['b'], c: ['c']}"><textarea class="disableshortkey"></textarea></div>`
      );
      vm.$mount(createDiv());
